  ...
  <div class="menu_nav_collapse" :class="sidebar">
    ...
    <el-menu :default-active="config.path" class="el-menu-vertical-demo" background-color="#001529" text-color="#fff"
      ...
      <template v-for="(item,index) in config.data">
        <!-- 二级 -->
        <el-submenu :index="item.link" v-if="item.child">
          <template slot="title">
            <span>{{item.name}}</span>
          </template>
          <el-menu-item-group>
            <el-menu-item v-for="_item in item.child" :index="_item.link" @click="router(item,_item)">{{_item.name}}</el-menu-item>
          </el-menu-item-group>
        </el-submenu>
        <!-- 级别一 -->
        <el-menu-item v-else :index="item.link" @click="router(item)">
